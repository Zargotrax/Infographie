...
void main() {
    vec3 normaleColor = texture(normalMap, texCoord).rgb * 2.0 - 1.0;;
    ...
    float reflection_diffuse_pointLight = max(dot(n+normaleColor, l_pointLight), 0.0);
    fragColor += vec4(diffuseColor * reflection_diffuse_pointLight * pointLight.color, 1.0);


    vec3 l_directionalLight = normalize(directionalLight.direction);
    float reflection_diffuse_directionalLight = max(dot(n+normaleColor, l_directionalLight), 0.0);
    ...
    fragColor += vec4(diffuseColor * reflection_diffuse_directionalLight * directionalLight.color, 1.0);


    vec3 l_projectorLight = normalize(projectorLight.position - vPosition);
    float reflection_diffuse_projectorLight = max(dot(n+normaleColor, l_projectorLight), 0.0);
    ...
    fragColor += vec4(diffuseColor * reflection_diffuse_projectorLight * projectedColor, 1.0);
}